...
object EspressoUtils {
    ...
    fun waitForViewToDisappear(
        ...
        timeoutMillis: Long = 10000, // Default timeout 10 seconds
    ...
    fun assertOnView(viewMatcher: Matcher<View?>?, assertion: ViewAssertion, waitMillis: Int = 10000, waitMillisPerTry: Long = 50) {
        val endTime = System.currentTimeMillis() + waitMillis
        while(System.currentTimeMillis() < endTime) {
            try {
                var view = Espresso.onView(viewMatcher)
        ...
        throw Exception("Error finding a view matching $viewMatcher within ${waitMillis}ms")
    }

    fun assertThatViewIsNotVisible() {
        return
    }
}
